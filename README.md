...
>
> 开发与维护边界见 [DEVELOPMENT.md](DEVELOPMENT.md)，其中说明输入搬运与顶部注入的独立语义。
>
...
- **管理控制台（浅色风格，静态 HTML/JS，免构建）**
  ...
  - **防截断（Anti-Truncation）**：下游请求体加 `"anti_truncation": true`（字段名可在控制台自定义）即对**该请求**启用"合成传输工具"包装——指示模型把最终回答放进工具参数输出，绕开重提示词场景（酒馆复杂预设/长历史）下的回答截断，代理透明还原为 `assistant.content`，真实工具调用不受影响（仅文本/Chat 模型适用）。
    - **真流式增量下发**（默认开，控制台可关）：请求上游把工具参数**分片**返回（`stream_function_call_arguments`），合成正文因此**逐片实时输出**——实测同一请求由"憋到最后一次性刷全文"（1 个分片、首字≈总时长）变为 24 个分片、**首字提前约 8 秒**。上游若不接受该字段（老模型/区域），代理会**自动降级**为整段下发并打一行告警，功能不受影响。
    - **真流式缓冲阈值**（默认 `0` = 直通）：`0` 时普通正文立刻下发，首字延迟最低；设为大于 0 的字节数则先把普通文本扣住、攒够即整批放行并永久转直通，命中合成调用时丢弃缓冲（正文以合成 content 为准），全程未命中则流末放行兜底。合成正文本身从不进缓冲。
    - 真实工具调用的分片参数会被**累积还原为完整 arguments** 后再下发，思考签名（Gemini 3.x 多轮必需）随首片逐字节保留，对客户端零行为变化。
  - **模型列表手动管理**：模型列表**不再自动拉取**远程配置（曾启动时 + `/v1/models` 每 3600s 自动刷新）；控制台「🌐 获取远程模型」手动刷新（结果持久化到磁盘）、「📝 编辑模型」弹窗添加/删除**自定义模型**（持久化，合并进 `/v1/models`）。
  - **渠道探活**（「通道与凭证」页顶部独立卡片）：一键检查**每一条**凭证现在还能不能用，解决"正式请求只告诉你这次失败了，不告诉你是哪条凭证坏了"。每条凭证两级判定：先本地结构校验（复用保存凭证时的同一套校验函数，零消耗、不合格就不发请求），再真发一次最小上游请求（`max_output_tokens=1`）。逐条列出 ✅/❌、耗时与失败归因（限流 / 权限计费 / Cookie 失效 / 项目未开计费 分开报，而不是笼统一句 403）。Cookie 通道走与正式请求**逐字同构**的 `batchGraphql` 请求体与解析器——HTTP 200 里包着的 GraphQL 错误信封同样判失败，不会误报通过。**只读**：不写状态文件、不进 Client 复用池、不碰熔断计数、不计入请求统计；并发 4、单条 20s 超时 + 26s 硬兜底，生图模型自动排除。手动触发，不做后台定时。
- **Gemini 能力与适配**
  - 文本对话、流式（SSE）与非流式。
...
- **预填充（重要，专为 SillyTavern 等酒馆预设优化）**：Gemini 3.x 拒绝以 `model` 轮次结尾的请求（返回 `Requests ending with a model turn are not supported.`）。代理内置"预填充智能兼容"（`smart`/`minimal`/`off`，默认 `smart`，控制台可切）：
  ...
  - **3.x 轮次出口兜底**：即使不是普通 assistant 纯文本预填充（例如末尾悬空 `tool_calls`、仅 `reasoning_content`、仅思考签名，或前端直接发送 `role=model`），转换后仍以 model 轮结尾也会触发同一个 400。代理在 Express/SA 共用的 `create_gemini_prompt` 与 Cookie 独立转换出口检查**转换结果**，对要求 user 收尾的 3.x 自动补一句极短 user 推动语；2.5 及更早保持原生 model 结尾透传。触发时日志为 `🩹 [轮次兜底]`，开启出站调试还会显示请求末条 role。
...
- **渠道探活**：同一页面顶部的独立卡片，一键逐条检查凭证是否仍然可用（两级判定：本地结构校验 + 真实最小请求）。详见上文「功能特性 → 渠道探活」。
